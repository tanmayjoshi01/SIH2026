...
/**
 * Detector / forecaster summary. Reads the Day 1 stub endpoints
 * (/api/anomalies, /api/predictions); Day 2 replaces the data behind
 * those routes without changing this component.
 */
  ...
  return (
    <section className="rounded-lg border border-slate-800 bg-slate-900 p-4">
      <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-200">
        ...
        <span className="rounded bg-slate-800 px-1.5 py-0.5 text-[10px] font-normal text-slate-400">stub</span>
      </h2>
              ...
              {predictions.map((item) => (
                <li key={item.id} className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <span className="tabular-nums text-slate-400">
                    {Math.round(item.failure_probability * 100)}% in ~{item.eta_minutes} min
                  </span>
                </li>
              ))}
            </ul>
